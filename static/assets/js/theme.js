...
document.getElementById("my_about_btn").addEventListener("click", function() {
  setTimeout(() => { docReady(countupInit); },2000);
});
...
var countupInit = function countupInit() {
  if (window.countUp) {
    ...
    countups.forEach(function (node) {
      ...
      var countUp = new window.countUp.CountUp(node, parseInt(node.innerText), _objectSpread({
        duration: 5
      }, options));
...
var navbarInit = function navbarInit() {
  ...
  document.getElementById("my_portfolio_btn").addEventListener("click", function() {
    if (portfolioGallery) {
      window.Isotope.data(portfolioGallery).arrange();
      console.log('my_portfolio_btn call is firing now');
    }
  });
